"""
Simple cleaner for Ukrainian texts

Source code of the tokenizer is borrowed from https://github.com/lang-uk/tokenize-uk/blob/languagetools/tokenize_uk/tokenize_uk.py
"""
...
parser.add_argument("--corpus-path", default="res_preprocessed.txt")
parser.add_argument("--corpus-clean", default="res_clean.txt")
...
def norm(s):
    ...
    for w in s.split(' '):
        if w != '-':
            w = w.replace('км²', 'кілометри квадратні')
            w = w.replace('км³', 'кілометри кубічні')
            w = w.replace('м³', 'метри кубічні')
            
            w = w.replace('млрд', 'мільярди')
            w = w.replace('км', 'кілометри')
...
def postfactum_strip(s):
    return s.replace('#', '').replace("#'",'<unk>').replace('#-го','<unk>')
...
def process_sentence(sent, min_words):
    tokens = [s  for s in t.tokenize(sent) if s != '' and s != ' ' and s != '(' and s != ')' and s != '—' and s != ',']
    words = [normalize_word(word) for word in tokens]
...
def replace_numeric(text, numeric_pattern=re.compile('[0123456789]+|,'), digit_pattern=re.compile('[0-9]|,'),
...
def main():
    ...
    for line in tqdm(lines, desc='applying cleaning jobs:'):
        ...
        batch_lines.append(obj.get())
        c += 1

        if c == 20_000:
            with open(args.corpus_clean, mode='a') as file:
                for bv in batch_lines:
                    for sentence in bv:
                        if sentence:
                            sentence_cleaned = postfactum_strip(sentence)
                            sentence_cleaned = norm(sentence_cleaned)
                            file.write(sentence_cleaned + "\n")
